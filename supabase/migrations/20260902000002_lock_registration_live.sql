-- Migration: Lock registration & cancellation when tournament is live/completed and protect paid registrations
...
begin
  ...
  select t.status, t.registration_open_at, t.registration_close_at, t.start_time, t.max_teams, t.title, coalesce(t.entry_fee_minor, 0)
    into current_status, open_at, close_at, start_at, max_allowed, tournament_title, fee_minor
  ...
  -- Disallow registration if closed, live, completed, cancelled, or outside registration window
  if now() < open_at or now() >= close_at or (start_at is not null and now() >= start_at) or current_status in ('registration_closed'::public.tournament_status, 'ongoing'::public.tournament_status, 'completed'::public.tournament_status, 'cancelled'::public.tournament_status) then
  ...
  if fee_minor > 0 then
    select count(*) into current_paid_count
      ...
      where tournament_id = target_tournament_id and payment_status = 'paid' and status <> 'cancelled';
  else
    select count(*) into current_paid_count
      from public.tournament_registrations
...
begin
  ...
  select title, status, registration_close_at, start_time
    into tournament_title, current_status, close_at, start_at
  ...
  if now() >= close_at or (start_at is not null and now() >= start_at) or current_status in ('registration_closed'::public.tournament_status, 'ongoing'::public.tournament_status, 'completed'::public.tournament_status, 'cancelled'::public.tournament_status) then
    raise exception 'registration_closed';
  end if;
  ...
  -- 2. Never cancel paid registrations (only pending registrations can be cancelled)
  if reg_record.payment_status = 'paid' then
    raise exception 'registration_closed';
  end if;

  -- 3. Mark registration and payment_status as cancelled without deleting row or altering payments table
  ...
  -- 4. If tournament was full, update status to open if slots became available
