...
def upgrade() -> None:
    ...
    op.alter_column(
        "users", "role_v2", new_column_name="role", nullable=False, server_default="user"
    )
    op.create_check_constraint(
        "ck_users_role", "users", "role IN ('user','organizer','admin','super_admin')"
    )
    ...
    op.create_table(
        ...
        sa.Column("banner_url", sa.String(500)),
        sa.Column("prize_pool_minor", sa.Integer(), nullable=False, server_default="0"),
        sa.Column("entry_fee_minor", sa.Integer(), nullable=False, server_default="0"),
        ...
        sa.Column("faqs", sa.JSON(), nullable=False, server_default="[]"),
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
            server_default=sa.text("now()"),
