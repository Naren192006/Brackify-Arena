...
export function AdminMatchApproval({ matchId }: { matchId: string }) {
  ...
  return <div className="mt-5 flex flex-wrap gap-3"><button className="btn-primary" disabled={review.isPending} onClick={() => review.mutate("approve")}>Approve result</button><button className="rounded-lg border border-white/10 px-4 py-2 text-sm text-arena-muted hover:text-white" disabled={review.isPending} onClick={() => review.mutate("reject")}>Reject result</button></div>;
}
