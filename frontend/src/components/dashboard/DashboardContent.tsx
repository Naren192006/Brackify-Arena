...
import { deleteTeam, getInvitations, getNotifications, getProfile, getRecentActivity, getTeams } from "@/lib/arena/data";
...
const card = "rounded-2xl border border-arena-border bg-arena-surface/80 p-4 sm:p-5 shadow-2xl shadow-black/10";
...
export function DashboardContent({ userId, email, metadata }: Props) {
  ...
  const deleteTeamMutation = useMutation({
    ...
    onSuccess: (_, teamId) => {
      ...
      void queryClient.invalidateQueries({ queryKey: ["teams", userId] });
      void queryClient.invalidateQueries({ queryKey: ["registered-tournaments", userId] });
  ...
  return (
    <div className="mx-auto max-w-[1500px] px-4 py-6 sm:px-8 sm:py-12 lg:py-16">
      <section className="glass-card dashboard-hero mb-6 sm:mb-8 overflow-hidden p-4 sm:p-8">
        ...
        <h1 className="font-display text-2xl sm:text-4xl lg:text-5xl font-bold text-arena-text">Welcome back, {name}</h1>
      ...
      <section id="my-teams" className={`${card} mt-8`}>
      ...
      <section className={`${card} mt-6`}><SectionTitle title="Fair play" /><div className="grid gap-3 sm:grid-cols-3"><div><p className="text-xs text-arena-muted">Trust score</p><p className="mt-1 font-display text-2xl font-bold text-arena-text">{reputationQuery.data?.trustScore ?? "—"}/100</p></div><div><p className="text-xs text-arena-muted">Active reports</p><p className="mt-1 font-display text-2xl font-bold text-arena-text">{reputationQuery.data?.activeReports ?? "—"}</p></div><div><p className="text-xs text-arena-muted">Warnings</p><p className="mt-1 font-display text-2xl font-bold text-arena-text">{reputationQuery.data?.warnings ?? "—"}</p></div></div></section>

      <section className={`${card} mt-6`}>
        <SectionTitle title="My teams" />
        <div className="grid gap-4 md:grid-cols-2">
          <div className="team-stack space-y-4">
            {teamsQuery.isLoading ? <LoadingRows /> : teamsQuery.data?.length ? teamsQuery.data.map((team) => (
              <TeamCard
                key={team.id}
                team={team}
                userId={userId}
                onInvite={(target) => invitePlayer.mutate({ teamId: team.id, target })}
                onDelete={(t) => setTeamToDelete(t)}
              />
            )) : <EmptyState title="No teams yet" text="Create your first roster and start building your competitive identity." />}
          </div>
          <CreateTeamForm pending={createTeam.isPending} onSubmit={(form) => createTeam.mutate(form)} />
        </div>
        {invitationsQuery.data?.length ? <div className="mt-6 border-t border-arena-border pt-5"><h3 className="mb-3 font-display text-xl font-semibold text-arena-text">Pending invitations</h3><div className="space-y-2">{invitationsQuery.data.map((invite) => <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-white/[0.04] p-3" key={invite.id}><span className="text-sm text-arena-muted">You&apos;ve been invited to <strong className="text-arena-text">{invite.teams?.name ?? "a team"}</strong></span><span className="flex gap-2"><button className="btn-primary px-3 py-1.5 text-xs" onClick={() => respond.mutate({ id: invite.id, decision: "accepted" })}>Accept</button><button className="rounded-lg border border-arena-border px-3 py-1.5 text-xs text-arena-muted hover:text-arena-text" onClick={() => respond.mutate({ id: invite.id, decision: "declined" })}>Decline</button></span></div>)}</div></div> : null}
      </section>
...
function SectionTitle({ title, action }: { title: string; action?: React.ReactNode }) { return <div className="mb-4 sm:mb-5 flex items-center justify-between"><h2 className="font-display text-xl sm:text-2xl font-semibold text-arena-text">{title}</h2>{action}</div>; }
...
function CreateTeamForm({ pending, onSubmit }: { pending: boolean; onSubmit: (form: FormData) => void }) { return <form id="create-team" action={onSubmit} className="glass-card create-team-panel p-4 sm:p-6"><p className="text-xs uppercase tracking-[0.24em] text-arena-accent">New roster</p><h3 className="mt-1.5 sm:mt-2 font-display text-2xl sm:text-3xl font-semibold text-arena-text">Create a team</h3><p className="mt-1.5 sm:mt-2 text-xs sm:text-sm text-arena-muted">You&apos;ll become captain and can invite players next.</p><div className="mt-4 sm:mt-6 space-y-3 sm:space-y-4"><input name="name" required minLength={2} maxLength={40} placeholder="Team name" className="glass-input w-full" /><input name="tag" required minLength={3} maxLength={6} placeholder="Team tag (3–6)" className="glass-input w-full uppercase" /><textarea name="description" maxLength={240} placeholder="Description (optional)" rows={3} className="glass-input w-full resize-none" /></div><button className="btn-primary mt-4 sm:mt-5 w-full" disabled={pending}>{pending ? "Creating…" : "Create team"}</button><div className="mt-3 sm:mt-4 flex flex-wrap gap-2 text-xs text-arena-muted"><span className="glass-badge">Captain access</span><span className="glass-badge">Up to 5 players</span><span className="glass-badge">Tag preview</span></div></form>; }
...
function TeamCard({
  ...
  const isCaptain = team.role === "captain" || team.captain_id === userId;
  const upload = async (file: File) => {
    setLogoBusy(true);
    const path = `${team.id}/${crypto.randomUUID()}-${file.name.replace(/[^a-zA-Z0-9._-]/g, "")}`;
    const { error } = await supabase.storage.from("team-logos").upload(path, file, { upsert: true, contentType: file.type });
    if (!error) {
      const { data } = supabase.storage.from("team-logos").getPublicUrl(path);
      const update = await supabase.from("teams").update({ logo_url: data.publicUrl }).eq("id", team.id);
      if (update.error) toast.error(update.error.message);
      else toast.success("Team logo updated");
  ...
  return (
    <TeamPreviewCard team={team}>
      <div className="mt-5 flex items-center justify-between gap-3">
        <p className="text-sm text-arena-muted">{team.description || "Build your competitive identity."}</p>
        <div className="flex items-center gap-2">
          <Link href={`/teams/${team.id}`} className="glass-badge shrink-0 text-arena-accent hover:border-arena-accent">View team</Link>
          {isCaptain ? (
            <button
              type="button"
              onClick={() => onDelete(team)}
              className="rounded-full border border-red-500/30 bg-red-500/10 px-3 py-1 text-xs font-medium text-red-400 hover:bg-red-500/20 hover:border-red-500/50 hover:text-arena-danger transition-colors shrink-0"
            >
              Delete Team
            </button>
          ) : null}
      ...
      {isCaptain ? (
        <form className="team-invite mt-5 flex gap-2" action={(form) => { const value = String(form.get("target") ?? "").trim(); if (value) { onInvite(value); setTarget(""); } }}>
          <div className="relative min-w-0 flex-1">
            <span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-arena-accent">⌕</span>
            <GlassInput name="target" value={target} onChange={(event) => setTarget(event.target.value)} placeholder="Invite by username or email" className="pl-10" />
          </div>
          <button className="btn-primary rounded-full px-4 py-2 text-sm">Invite</button>
        </form>
      ) : null}
      <div className="mt-4 flex flex-wrap items-center gap-3">
        {isCaptain ? (
          <label className="cursor-pointer text-xs text-arena-accent hover:underline">
            {logoBusy ? "Uploading…" : "Update logo"}
            <input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" disabled={logoBusy} onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); }} />
          </label>
        ) : null}
        <span className="text-xs text-arena-muted">Members and management available from View team.</span>
      </div>
    </TeamPreviewCard>
  );
}
