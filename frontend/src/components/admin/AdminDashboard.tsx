...
const card = "rounded-2xl border border-arena-border bg-arena-surface/80 p-5 shadow-lg backdrop-blur-md";
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 space-y-8">
      <div>
        ...
        <h1 className="mt-2 font-display text-4xl font-bold text-arena-text">Admin dashboard</h1>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {cards.map(([label, value]) => (
          <div className={card} key={String(label)}>
            ...
            <p className="mt-2 font-display text-3xl font-bold text-arena-text">{value}</p>
          </div>
        ))}
      </div>

      <section className={`${card} mt-6`}>
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-arena-border pb-4">
          <h2 className="font-display text-xl font-semibold text-arena-text">Quick actions</h2>
          <div className="flex flex-wrap gap-2">
            <Link className="btn-primary px-3.5 py-2 text-xs font-semibold" href="/admin/tournaments/create">
              Create tournament
            </Link>
            <Link className="rounded-xl border border-arena-border bg-arena-bg-elevated px-3.5 py-2 text-xs font-semibold text-arena-text hover:bg-arena-bg-elevated transition-colors" href="/admin/tournaments">
              View tournaments ({totalTournaments})
            </Link>
            <Link className="rounded-xl border border-arena-border bg-arena-bg-elevated px-3.5 py-2 text-xs font-semibold text-arena-muted hover:text-arena-text transition-colors" href="/admin/reports">
              Review reports
            </Link>
          </div>
        </div>

        <h2 className="mt-6 font-display text-lg font-semibold text-arena-text">Recent tournaments</h2>
        <div className="mt-4 space-y-2">
          {tournamentsQuery.isLoading ? (
            ...
            list.slice(0, 8).map((item) => (
              <Link
                className="flex items-center justify-between rounded-xl bg-white/[0.03] p-3.5 border border-arena-border hover:bg-white/[0.06] hover:border-arena-border transition-colors"
                href={`/admin/tournaments/${item.slug || item.id}`}
                key={item.id}
              >
                <div>
                  <span className="font-medium text-arena-text text-sm">{item.title}</span>
