...
function getStatusBadge(status: string) {
  switch (status) {
    ...
    case "investigating":
      return "border-cyan-400/40 bg-cyan-400/15 text-arena-accent";
    case "resolved":
      return "border-emerald-400/40 bg-emerald-400/15 text-emerald-400";
    case "rejected":
      return "border-white/10 bg-white/5 text-arena-muted";
    case "banned":
      return "border-red-500/50 bg-red-500/20 text-red-300 font-bold";
    default:
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 space-y-6">
              ...
              <button
                ...
                className={`rounded-xl px-4 py-2 text-xs font-semibold transition-colors ${
                  activeFilter === val
                    ...
                    : "border border-white/10 bg-white/[0.02] text-arena-muted hover:text-white"
                }`}
              >
                {label}
              </button>
      ...
      <section className="overflow-x-auto rounded-3xl border border-white/10 bg-[#0a0e1a]/80 shadow-2xl backdrop-blur-xl">
        <table className="w-full min-w-[950px] text-left text-xs">
          <thead className="border-b border-white/10 bg-white/[0.02] text-arena-muted font-mono uppercase tracking-wider text-[10px]">
            <tr>
            ...
            {reportsQuery.isLoading ? (
              ...
              <tr>
                <td colSpan={7} className="px-5 py-16 text-center text-arena-muted">
                  ...
                  <p className="font-semibold text-white">No reports found in this status queue.</p>
              ...
              reports.map((report) => (
                <tr key={report.id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                      {report.reporter_team?.logo_url ? (
                        <img
                          ...
                          alt=""
                          className="h-8 w-8 rounded-lg border border-white/10 bg-black/40 object-cover"
                        />
                      ) : (
                        <div className="flex h-8 w-8 items-center justify-center rounded-lg border border-white/10 bg-white/5 font-bold text-white">
                  ...
                  <td className="px-5 py-4 max-w-xs">
                    <span className="inline-block rounded-md border border-white/10 bg-white/5 px-2 py-0.5 text-[11px] font-semibold text-white">
                      {report.reason}
                    </span>
                    ...
                    {report.admin_notes ? (
                      <div className="mt-1 rounded border border-cyan-400/20 bg-cyan-400/5 p-1.5 text-[10px] text-cyan-300">
                  ...
                  <td className="px-5 py-4">
                    {report.tournament ? (
                      <Link
                        ...
                        className="font-semibold text-white hover:text-arena-accent transition-colors line-clamp-1"
                      >
                    ...
                    <div className="flex items-center justify-end gap-1.5">
                      {report.status !== "resolved" ? (
                        <button
                          ...
                          className="rounded-lg border border-emerald-500/40 bg-emerald-500/15 px-2.5 py-1 text-[11px] font-semibold text-emerald-300 hover:bg-emerald-500/25 transition-colors disabled:opacity-50"
                          title="Mark Resolved"
                        >
                          Resolve
                        </button>
                      ...
                      {report.status !== "rejected" ? (
                        <button
                          ...
                          className="rounded-lg border border-white/10 bg-white/[0.04] px-2.5 py-1 text-[11px] font-semibold text-arena-muted hover:text-white transition-colors disabled:opacity-50"
                          title="Reject Report"
                        >
                          Reject
                        </button>
                      ...
                      {report.status !== "banned" ? (
                        <button
                          ...
                          className="rounded-lg border border-red-500/40 bg-red-500/15 px-2.5 py-1 text-[11px] font-semibold text-red-300 hover:bg-red-500/25 transition-colors disabled:opacity-50"
                      ...
                      <button
                        ...
                        className="rounded-lg border border-cyan-400/30 bg-cyan-400/10 px-2.5 py-1 text-[11px] font-semibold text-arena-accent hover:bg-cyan-400/20 transition-colors"
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between border-t border-white/10 pt-4 px-2">
          <p className="text-xs text-arena-muted">
            Showing <span className="font-semibold text-white font-mono">{reports.length}</span> of{" "}
            <span className="font-semibold text-white font-mono">{totalCount}</span> reports (Page{" "}
          ...
          <div className="w-full max-w-md rounded-3xl border border-white/15 bg-[#0d121f] p-6 shadow-2xl space-y-4 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="space-y-3 text-xs">
              <p className="text-arena-muted">
                ...
                <span className="text-white font-semibold">{selectedReportForNotes.reason}</span>
              </p>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <button
                type="button"
                onClick={() => setSelectedReportForNotes(null)}
                className="rounded-xl border border-white/10 px-4 py-2 text-xs text-arena-muted hover:text-white"
              >
                Cancel
              </button>
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-3xl border border-red-500/40 bg-[#0d121f] p-6 shadow-2xl space-y-4 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="space-y-2 text-xs text-arena-muted">
              ...
              <div className="rounded-xl border border-red-500/20 bg-red-950/20 p-3 text-white">
              ...
              <p className="text-[11px]">
                This will mark the fair play report as <span className="text-red-300 font-bold uppercase">BANNED</span> and notify tournament arbiters.
              </p>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t border-white/10">
              ...
              <button
                ...
                className="rounded-xl border border-red-500/50 bg-red-600 px-5 py-2 text-xs font-bold text-white hover:bg-red-500 disabled:opacity-50 transition-colors shadow-lg shadow-red-950/50"
              >
