...
export function MatchManager({ tournamentId, matches }: { tournamentId: string; matches: AdminMatch[] }) {
  ...
  return <div className="space-y-3">{matches.length ? matches.map((match) => {
    ...
    return <article className="rounded-xl border border-arena-border bg-white/[0.04] p-4" key={match.id}>
      <div className="flex flex-wrap items-center justify-between gap-3"><p className="font-semibold text-arena-text">Round {match.round_number} · Match {match.match_number}</p><span className="text-xs uppercase text-arena-accent">{match.status.replaceAll("_", " ")}</span></div>
      <div className="mt-2 grid gap-1 text-sm text-arena-text sm:grid-cols-2"><span>Team A: {match.team_a?.name ?? shortId(match.team_a_id)}</span><span>Team B: {match.team_b?.name ?? shortId(match.team_b_id)}</span></div>
