...
export function AdminAssignment({ tournamentId }: { tournamentId: string }) {
  ...
  return <div className="space-y-3">{users.data?.map((user) => { const assigned = admins.data?.some((admin) => admin.user_id === user.user_id); return <div className="flex items-center justify-between rounded-xl bg-white/[0.04] p-3" key={user.user_id}><span className="text-sm text-arena-text">{user.display_name || user.username || user.user_id}</span>{assigned ? <button className="text-xs text-arena-muted hover:text-arena-text" onClick={() => remove.mutate(user.user_id)}>Remove</button> : <button className="text-xs text-arena-accent hover:underline" onClick={() => assign.mutate(user.user_id)}>Assign</button>}</div>; })}</div>;
}
