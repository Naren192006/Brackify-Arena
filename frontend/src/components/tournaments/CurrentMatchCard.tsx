...
export function CurrentMatchCard({ match, teamId, tournamentSlug }: { match: Match; teamId: string; tournamentSlug: string }) {
  ...
  const opponent = useQuery({
    queryKey: ["match-opponent", opponentId],
    queryFn: async () => {
      if (!opponentId) return null;
      const { data, error } = await supabase.from("teams").select("name,tag").eq("id", opponentId).maybeSingle();
  ...
  return (
    <article className="rounded-xl border border-cyan-400/20 bg-cyan-400/10 p-4">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1.5 text-xs text-arena-muted">
            {match.scheduled_at ? new Date(match.scheduled_at).toLocaleString() : "Schedule to be announced"}
          </p>
        </div>
        <div className="self-start sm:self-auto">
          <StatusBadge status={awaiting ? "awaiting_approval" : match.status} />
        </div>
      </div>
      ...
      {awaiting ? (
        <p className="mt-3 text-xs sm:text-sm text-arena-muted">Waiting for opponent or admin approval.</p>
      ) : match.status === "live" ? (
        <div className="mt-3 w-full">
          <SubmitResultModal matchId={match.id} teamIsA={match.team_a_id === teamId} />
        </div>
      ) : match.status === "scheduled" || match.status === "pending" ? (
        <p className="mt-3 text-xs sm:text-sm text-arena-muted">Waiting for the admin to start this match.</p>
      ) : null}
      <Link href={`/tournaments/${tournamentSlug}/matches/${match.id}`} className="mt-3 inline-block text-xs text-arena-accent hover:underline">
        Open match →
      </Link>
    </article>
  );
