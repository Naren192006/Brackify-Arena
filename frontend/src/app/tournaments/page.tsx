...
function TournamentCard({ tournament }: { tournament: Tournament }) { const remaining = Math.max(0, tournament.max_teams - tournament.registered_count); return <Link href={`/tournaments/${tournament.slug}`} className="glass-card overflow-hidden rounded-xl transition-transform hover:-translate-y-1">{tournament.banner_url ? <img src={tournament.banner_url} alt="" className="h-36 w-full object-cover" /> : <div className="arena-grid-bg h-36 bg-cyan-400/10" />}<div className="p-6"><div className="flex items-center justify-between gap-3"><p className="text-xs uppercase tracking-widest text-arena-accent">{tournament.game} · {tournament.mode}</p><StatusBadge status={tournament.status} registeredCount={tournament.registered_count} maxTeams={tournament.max_teams} registrationOpenAt={tournament.registration_open_at} registrationCloseAt={tournament.registration_close_at} /></div><h2 className="mt-3 font-display text-2xl font-semibold text-white">{tournament.title}</h2><p className="mt-3 line-clamp-2 text-sm text-arena-muted">{tournament.description || "Compete for your place in the arena."}</p><div className="mt-6 flex justify-between text-sm text-arena-muted"><span>{remaining} slots left</span><span>{new Date(tournament.start_time).toLocaleDateString()}</span></div></div></Link>; }
